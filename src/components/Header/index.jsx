...
import IconBxHome from "./icons/IconBxHome";
import IconBxNews from "./icons/IconBxNews";
import IconCommunityLine from "./icons/IconCommunityLine";
import IconShoppingCart from "./icons/IconShoppingCart";
import IconWpforms from "./icons/IconWpforms";
              ...
              <div className="header-menu-options flex-wrap">
                <li className="header-menu dropdown">
                  ...
                  <ul className="dropdown-content">
                    <Link
                      to="/me"
                      place={`Principal: ${user?.username} - ${config.hotel.name}`}
                    ...
                    <Link
                      ...
                      place={`Perfil: ${user?.username} - ${config.hotel.name}`}
                      className="list-content"
                    >
                ...
                <li className="header-menu dropdown">
                  ...
                  <ul className="dropdown-content">
                    {Array.isArray(config.newsSubmenu) &&
                      config.newsSubmenu.length > 0 &&
                      config.newsSubmenu.map((item, index) => {
                        return (
                          <Link key={index} to={`/news/${item.newsId}`} className="list-content">
                ...
                <li className="header-menu dropdown">
                  ...
                  <div className="dropdown-content">
                    <Link
                      to="/community/hall/coins"
                      place={`Comunidade: Hall da Fama  - ${config.hotel.name}`}
                      className="list-content"
                    >
                    ...
                    <Link
                      to="/community/staff"
                      place={`Comunidade: Equipe  - ${config.hotel.name}`}
                      className="list-content"
                    >
                    ...
                    <Link
                      to="/community/photos"
                      place={`Comunidade: Fotos  - ${config.hotel.name}`}
                      className="list-content"
                    >
                      <h5 className="no-select">{i18n.t("header.dropdown.community.photos")}</h5>
                    </Link>
                  </div>
                </li>
        ...
        <div className="webcenter flex">
          <div className="mr-bottom-2 margin-top-min width-content">
            <div className="general-box hotel-big-msg flex width-content">
              ...
              <h5 className="margin-auto-top-bottom">{i18n.t("index.header")}</h5>
            </div>
          </div>
        </div>
      )}
